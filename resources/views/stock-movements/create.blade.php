                ...
                <form action="{{ route('stock-movements.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="jenis" id="jenis"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            <option value="masuk" {{ old('jenis') == 'masuk' ? 'selected' : '' }}>Masuk (dari Dealer)</option>
                            <option value="keluar" {{ old('jenis') == 'keluar' ? 'selected' : '' }}>Keluar (Terjual)</option>
                        </select>
                        @error('jenis')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="keterangan" id="keterangan" rows="3"
                                  placeholder="Contoh: Pengiriman dari dealer utama, atau nama pembeli"
                                  class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('keterangan') }}</textarea>
                        @error('keterangan')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
